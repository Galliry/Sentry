...
void _Data_Diapcak(uint8_t *pdata)
{
	...
	if(cmd_id==0x0207)
	{
		...
		referee2022.shoot_data.bullet_type = *(pdata+data_addr); //弹丸类型
		referee2022.shoot_data.shooter_id =	*(pdata+data_addr + 1); //发射机构id	
		referee2022.shoot_data.bullet_freq = *(pdata+data_addr + 2); //弹丸射速

		BYTE0(referee2022.shoot_data.bullet_speed) = *(pdata+data_addr + 3); //弹丸初速度
	...
	if(cmd_id==0x020D)
	{
		...
		referee2022.sentry_info_t.free_revival = (referee2022.sentry_info_t.sentry_info >> 19) & 1;		//是否可以免费复活
		
		referee2022.sentry_info_t.out_of_combat = referee2022.sentry_info_t.sentry_info_2 & 1;		//是否脱战
...
void Sentry_multiMachineInteraction(void)
{
	...
	referee2022.robot_interactive_data.header.data_length[0]=0x2F;
	...
	referee2022.robot_interactive_data.data_cmd_id[0]=0x22;
	...
	if(referee2022.game_robot_status.robot_id==0x07)
	{
		...
		referee2022.robot_interactive_data.receiver_ID[0]=0x09;
		referee2022.robot_interactive_data.receiver_ID[1]=0x00;
	}
	else if(referee2022.game_robot_status.robot_id==0x6b)
	{
		referee2022.robot_interactive_data.sender_ID[0]=0x06b;
		referee2022.robot_interactive_data.sender_ID[1]=0x00;
...
//	referee2022.robot_interactive_data.data[3] = 0x01;
	...
	memcpy(&referee2022.robot_interactive_data.data[32],&referee2022.game_robot_pos.x,4);
	memcpy(&referee2022.robot_interactive_data.data[36],&referee2022.game_robot_pos.y,4);
	referee2022.robot_interactive_data.data[40] = 0x01;
	crc16_temp=Get_CRC16_Check_Sum((unsigned char*)&referee2022.robot_interactive_data,54, 0xFFFF);
	referee2022.robot_interactive_data.CRC16[0]=crc16_temp&0xff;
	referee2022.robot_interactive_data.CRC16[1]=crc16_temp>>8;
	
	memcpy(Data_Pack[1],(unsigned char*)&referee2022.robot_interactive_data,sizeof(referee2022.robot_interactive_data));
	length = sizeof(referee2022.robot_interactive_data);
	HAL_UART_Transmit_DMA(&huart4,Data_Pack[1],56);
}
...
void Sentry_To_Referee(void)
{
	...
	if(i == 0)
	{
		Sentry_Decision_Control();
		i++;
	}else if(i == 1)
	{
		Sentry_multiMachineInteraction();
		i = 0;
	}
}

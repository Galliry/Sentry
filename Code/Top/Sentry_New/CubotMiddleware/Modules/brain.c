...
void Brain_Autoaim_DataUnpack(Brain_t* brain ,uint8_t * recBuffer)
{
	...
	if (recBuffer[0] == 0xAA)
	{
		...
		if ((brain->Autoaim.Brain_Data.FrameType == 1) && recBuffer[12] == 0xDD) //< 瑙ｇ畻鍋忚浆锟??
		{
			...
			if(rc_Ctrl_et.rc.s2 == 2)
			{
				...
				#endif
				#if HOLDER_MODE == 2
				// 姝ゆ椂鍜岃嚜鐬勭殑閫氫俊鍗忥拷??涓猴細鍘熷厛琛ㄧず澧為噺鐨勬暟锟??鍙樹负浣嶇疆鐨勬暟锟??
...
void Brain_Lidar_DataUnpack(Brain_t* brain ,uint8_t * recBuffer)
{
	if (recBuffer[0] == 0xAA)
	{
		...
		if (brain->Lidar.Brain_Data.FrameType == BRAIN_TO_ROBOT_CMD) //< 瑙ｇ畻鍋忚浆锟??
		{
...
void RobotToBrain_Autoaim(float yaw,Brain_t* brain)//鍙戠粰锟??锟??
{
	...
	RobotToBrainTimeBuffer[1] = 0x07;	// Type ;  //鍥哄畾锟??0x07
	RobotToBrainTimeBuffer[2] = 0x01;	// coreID;  //锟??鍓嶅浐瀹氫负0x01
	RobotToBrainTimeBuffer[3] = 0x01;	// 绱㈠紩锛宨nt16_t锟??
	RobotToBrainTimeBuffer[4] = 0x01;

	RobotToBrainTimeBuffer[5] = (tim14.ClockTime >> 24);	// 瀹氭椂鍣ㄦ椂闂达紝int32_t锟??
...
void RobotToBrain_Lidar(Brain_t* Brain)
{
	...
	RobotToBrainChassisTimeBuffer[5] = Top.Referee.base_flag; //淇濇姢Base 锟??璁や负1

...
void PID_Init(void)
{
    BasePID_Init(&pid_load_angle, 6, 300, -8, 480, 30.0f);         // 拨弹盘角度
    BasePID_Init(&pid_load_speed, 90,0,0,0,30.0f);// 拨弹盘速度
    BasePID_Init(&pid_friction0, 10, 1.5, 2, 0 ,30.0f); // 摩擦轮
    BasePID_Init(&pid_friction1, 10, 1.5, 2, 0 , 30.0f);
    BasePID_Init(&pid_pitch_angle, 0.25, 0.34, -0.3, 3.2, 30.0f); // 云台 0.5 0.005 -0.005 1.5
    BasePID_Init(&pid_pitch_speed, 0.8, 4, 0.25, 0.25, 30.0f);
    BasePID_Init(&pid_yaw_s_angle, 0.45, 8, 0.3, 5 , 30.0f);//0.8 0.0045 -4 打符PID
